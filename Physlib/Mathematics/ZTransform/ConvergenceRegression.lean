...
regression row T-03 of `goal.md` section I.3. It is a strengthening of the sources: both
U. Siddique, M. Y. Mahmoud, and S. Tahar, "On the Formalization of Z-Transform in HOL",
ITP 2014, LNCS 8558, Definition 9 (p. 488), and the journal version, "Formal Analysis of
Discrete-Time Systems using z-Transform", Journal of Applied Logics 5(4), 2018, define the
region of convergence by summability, that is absolutely, and neither names a conditional
region or proves the two apart.
...
lemma seriesTerm_harmonicSeq_neg_one (n : ℕ) :
    seriesTerm harmonicSeq (-1) n = (((-1 : ℝ) ^ n * (((n + 1 : ℕ) : ℝ))⁻¹ : ℝ) : ℂ) := by
...
theorem not_summable_seriesTerm_harmonicSeq :
...
theorem neg_one_notMem_ROC_harmonicSeq : (-1 : ℂ) ∉ ROC harmonicSeq := by
  intro h
...
theorem neg_one_mem_condROC_harmonicSeq : (-1 : ℂ) ∈ condROC harmonicSeq := by
...
conditional one. -/
theorem ROC_ssubset_condROC_harmonicSeq : ROC harmonicSeq ⊂ condROC harmonicSeq :=
...
theorem exists_ROC_ssubset_condROC : ∃ f : ℤ → ℂ, ROC f ⊂ condROC f :=
...
theorem self_notMem_ROC_geometricSeq {a : ℂ} (ha : a ≠ 0) : a ∉ ROC (geometricSeq a) := by
  rw [ROC_geometricSeq ha]
  simp

/-- A point strictly outside the circle of radius `‖a‖` is in the region of convergence. -/
theorem two_mul_mem_ROC_geometricSeq {a : ℂ} (ha : a ≠ 0) : 2 * a ∈ ROC (geometricSeq a) := by
...
theorem transform_geometricSeq_half : transform (geometricSeq (1 / 2)) 1 = 2 := by
...
theorem one_notMem_ROC_unitStep : (1 : ℂ) ∉ ROC unitStep := by
  rw [ROC_unitStep]
  simp

/-- The unit step diverges at `Complex.I`, which has the same modulus as `1`. -/
theorem I_notMem_ROC_unitStep : Complex.I ∉ ROC unitStep := by
  rw [ROC_unitStep]
  simp

/-- The unit step converges at `2 * Complex.I`, which lies on the same ray as `Complex.I` but has
larger modulus. -/
theorem two_mul_I_mem_ROC_unitStep : 2 * Complex.I ∈ ROC unitStep := by
  rw [ROC_unitStep]
