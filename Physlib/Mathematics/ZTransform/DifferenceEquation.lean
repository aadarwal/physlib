...
transform theory. It is not needed to transform the equation, and it is exactly what makes the
equation determine its causal solution: two causal solutions driven by the same input are equal.
...
Journal of Applied Logics 5(4), 2018, pp. 875-907, Definitions 15-18 and Theorems 13-14
...
Three differences from those sources are recorded. First, the sources index coefficients by
...
theorem transform_delayCombination (hf : IsCausal f) (hs : Summable (seriesTerm f z))
    (s : Finset ℕ) (c : ℕ → ℂ) :
...
theorem transform_isRecurrenceSolution (hx : IsCausal x) (hy : IsCausal y)
...
theorem eq_of_isRecurrenceSolution {y₁ y₂ : ℤ → ℂ} (h0 : 0 ∉ s) (hc₁ : IsCausal y₁)
...
the input transform. -/
theorem transform_eq_transferFunction_mul (hx : IsCausal x) (hy : IsCausal y)
    (hxs : Summable (seriesTerm x z)) (hys : Summable (seriesTerm y z))
...
theorem transform_eq_transferFunction_mul_of_mem_iirROC (hx : IsCausal x) (hy : IsCausal y)
...
theorem transferFunction_eq_div (hx : IsCausal x) (hy : IsCausal y)
    (hxs : Summable (seriesTerm x z)) (hys : Summable (seriesTerm y z))
