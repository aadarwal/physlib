...
first samples. -/
theorem convolution_natCast (hx : IsCausal x) (h : ℤ → ℂ) (n : ℕ) :
...
theorem convolution_unitImpulse_left (x : ℤ → ℂ) : convolution unitImpulse x = x := by
  funext n
...
theorem seriesTerm_convolution (hx : IsCausal x) (h : ℤ → ℂ) (z : ℂ) (n : ℕ) :
...
theorem summable_seriesTerm_convolution (hx : IsCausal x) (hh : Summable (seriesTerm h z))
...
theorem transform_convolution (hx : IsCausal x) (hh : Summable (seriesTerm h z))
...
theorem mem_ROC_convolution (hx : IsCausal x) (hh : z ∈ ROC h) (hxz : z ∈ ROC x) :
...
theorem convolution_unitImpulse_right (h : ℤ → ℂ) (m : ℕ) :
...
theorem eq_natCast_of_convolution_unitImpulse_eq {h₁ h₂ : ℤ → ℂ}
    (heq : convolution h₁ unitImpulse = convolution h₂ unitImpulse) (m : ℕ) : h₁ m = h₂ m := by
...
identified by its impulse response. -/
theorem eq_of_isCausal_of_convolution_unitImpulse_eq {h₁ h₂ : ℤ → ℂ} (hc₁ : IsCausal h₁)
