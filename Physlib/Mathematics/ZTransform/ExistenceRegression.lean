...
Because a causal solution now exists and is unique, the transfer relation is a statement about
the difference equation itself rather than about a solution someone supplies. The second example
states it in that form for the one-pole recurrence.
...
causal. -/
theorem zero_notMem_onePole_lags : (0 : ℕ) ∉ ({1} : Finset ℕ) := by decide
...
theorem recurrenceSolution_onePole (a : ℂ) :
...
theorem transform_recurrenceSolution_onePole {a z : ℂ} (ha : a ≠ 0) (hz : ‖a‖ < ‖z‖)
...
transform. -/
theorem transform_recurrenceSolution_onePole_eq {a z : ℂ} (ha : a ≠ 0) (hz : ‖a‖ < ‖z‖) :
    transform (recurrenceSolution {1} {0} (onePoleFeedback a) onePoleFeedforward unitImpulse) z
      = (1 - a * z⁻¹)⁻¹ := by
