...
Absolute convergence of the transform series at `z` depends on `z` only through `‖z‖`, because
the norm of the `n`-th term is `‖f n‖ * ‖z‖⁻¹ ^ n`. This file proves that, deduces that the
absolute region of convergence is invariant under rotation and closed upward in modulus, and
concludes that a nonempty absolute region of convergence contains the exterior of a circle.

The upward closure is proved with a non-strict inequality, `‖w‖ ≤ ‖z‖`, by direct comparison of
the term norms. No boundedness or limit argument is required, and the conclusion is stronger
than the usual Abel-type statement, which assumes a strict inequality.

The substitution `u = z⁻¹` turns the transform into an ordinary power series `∑' n, f n * u ^ n`.
That identity is recorded here because it is the bridge to inverse-transform and uniqueness
results, which are developed separately.

The concrete sequences are the causal unit step and the causal geometric sequence. Both have an
absolute region of convergence computed exactly, not merely bounded: `‖z‖ > 1` for the step and
`‖z‖ > ‖a‖` for the geometric sequence with ratio `a ≠ 0`. The geometric transform is obtained
from the step transform through the `z`-domain scaling law rather than resummed.
...
The exterior-of-a-circle shape of the region of convergence is Definition 19 (p. 893) of
U. Siddique, M. Y. Mahmoud, and S. Tahar, "Formal Analysis of Discrete-Time Systems using
z-Transform", Journal of Applied Logics 5(4), 2018, pp. 875-907, together with the three
region-shape lemmas stated there. `IsExteriorOfCircle` keeps that source's `0 < R` form.

The unit step and the geometric sequence with their regions of convergence are entries of
Table 1 (p. 888) of the same paper. A textbook reference is A. V. Oppenheim and R. W. Schafer,
*Discrete-Time Signal Processing*, 3rd ed., Pearson, 2010, chapter 3.

Two results here are stronger than the source statements. First, the source propagates absolute
convergence outward under a strict modulus inequality; `summable_seriesTerm_of_norm_le` proves it
under a non-strict one, so the region of convergence is also proved invariant under rotation.
Second, the regions of convergence of the unit step and the geometric sequence are computed
exactly, as set equalities, rather than only shown to contain an exterior.
...
lemma summable_seriesTerm_of_norm_le {f : ℤ → ℂ} {w z : ℂ} (hw : w ≠ 0) (hle : ‖w‖ ≤ ‖z‖)
...
lemma ROC_mem_of_mem_of_norm_le {f : ℤ → ℂ} {w z : ℂ} (hw : w ∈ ROC f) (hle : ‖w‖ ≤ ‖z‖) :
    z ∈ ROC f :=
...
lemma ROC_mem_of_mem_of_norm_eq {f : ℤ → ℂ} {w z : ℂ} (hw : w ∈ ROC f) (heq : ‖w‖ = ‖z‖) :
    z ∈ ROC f :=
  ROC_mem_of_mem_of_norm_le hw heq.le

/-- A set of complex numbers contains the exterior of a circle of positive radius. -/
def IsExteriorOfCircle (s : Set ℂ) : Prop := ∃ R : ℝ, 0 < R ∧ ∀ z : ℂ, R < ‖z‖ → z ∈ s
...
nonempty. -/
theorem isExteriorOfCircle_ROC_iff {f : ℤ → ℂ} :
...
lemma transform_inv (f : ℤ → ℂ) (u : ℂ) : transform f u⁻¹ = ∑' n : ℕ, f n * u ^ n := by
  rw [transform]
...
theorem ROC_unitStep : ROC unitStep = {z : ℂ | 1 < ‖z‖} := by
  ext z
  constructor
  · rintro ⟨hz, hs⟩
...
theorem transform_unitStep {z : ℂ} (hz : 1 < ‖z‖) : transform unitStep z = (1 - z⁻¹)⁻¹ := by
...
theorem ROC_geometricSeq {a : ℂ} (ha : a ≠ 0) :
...
radius `‖a‖`. -/
theorem transform_geometricSeq {a z : ℂ} (ha : a ≠ 0) (hz : ‖a‖ < ‖z‖) :
