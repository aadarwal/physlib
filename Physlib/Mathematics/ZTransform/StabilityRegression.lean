...
The last example is a two-pole recurrence with a repeated pole at `2⁻¹`. It is Schur stable,
and its feedback coefficients have total modulus `5 / 4`, which is not less than one. So the
sufficient coefficient criterion of `Physlib.Mathematics.ZTransform.Stability` does not apply to
it, and the criterion is therefore proved to be strictly sufficient rather than necessary.
...
- `Physlib.ZTransform.isSchurStable_twoPole`: a Schur-stable two-pole recurrence.
...
theorem candidatePoles_onePole {a : ℂ} (ha : a ≠ 0) :
...
theorem isSchurStable_onePole_iff {a : ℂ} (ha : a ≠ 0) :
...
modulus below one. -/
theorem isAbsSummable_geometricSeq {a : ℂ} (ha : a ≠ 0) :
...
theorem sphere_subset_ROC_geometricSeq {a : ℂ} (ha : a ≠ 0) (h : ‖a‖ < 1) :
...
theorem isBIBOStable_geometricSeq {a : ℂ} (ha : a ≠ 0) (h : ‖a‖ < 1) :
...
theorem isSchurStable_onePole_of_norm_lt_one {a : ℂ} (h : ‖a‖ < 1) :
...
/-- The feedback coefficients of a two-pole recurrence with a repeated pole at `2⁻¹`. -/
...
/-- The two-pole recurrence is Schur stable: its only candidate pole is `2⁻¹`. -/
theorem isSchurStable_twoPole : IsSchurStable {1, 2} twoPoleFeedback := by
  rintro z ⟨hne, hzero⟩
  ...
  have hzval : z = 2⁻¹ := by rw [← hz, inv_inv]
  rw [hzval, norm_inv]
  norm_num
...
theorem not_sum_norm_lt_one_twoPole : ¬ (∑ k ∈ ({1, 2} : Finset ℕ), ‖twoPoleFeedback k‖) < 1 := by
