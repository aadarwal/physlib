...
The estimate that drives everything is elementary. If the transform converges absolutely at
`w`, then for every `z` of modulus at least `‖w‖` the tail beyond the first sample is bounded by
...
The results are Physlib alternatives related to Theorem 15 (inverse z-transform, p. 894),
Theorem 16 (uniqueness, p. 894), and Theorem 17 (initial value, p. 894) of U. Siddique,
M. Y. Mahmoud, and S. Tahar, "Formal Analysis of Discrete-Time Systems using z-Transform",
Journal of Applied Logics 5(4), 2018, pp. 875--906. Those three theorems are absent from the
earlier conference version, "On the Formalization of Z-Transform in HOL", ITP 2014, LNCS 8558,
whose section 6 (p. 497) lists them as future work.

Two differences from the source are material. First, the source recovers the samples as
...
`tendsto_inversion_cobounded`, which also recovers every sample but is not the source theorem's
literal formula. The Taylor-coefficient form is not proved here; it would need the power series in
the reciprocal variable to be exhibited as an analytic function with its radius of convergence,
and is recorded as remaining work rather than claimed.

Second, the source's theorems use an exterior-circle hypothesis for its ordered-summability region
of convergence. The results below instead start from a witness in Physlib's absolute region of
convergence and, for uniqueness, assume equality on the corresponding closed exterior. Thus they
are convergence-stronger alternatives rather than literal source-parity statements. The source
also uses one-sided sequences, so causality is implicit in its setting. Here sequences are
two-sided and the unilateral transform cannot see a negative index, so uniqueness of the sequence
itself requires causality explicitly. Without it only the nonnegative-index samples are
determined, and that weaker statement is what `eq_natCast_of_transform_eqOn` asserts.
...
lemma norm_seriesTerm_succ_le
    (f : ℤ → ℂ) (hw : w ≠ 0) (hle : ‖w‖ ≤ ‖z‖) (n : ℕ) :
...
lemma norm_transform_sub_apply_zero_le (hw : w ∈ ROC f) (hle : ‖w‖ ≤ ‖z‖) :
...
lemma tendsto_transform_cobounded (hw : w ∈ ROC f) :
...
lemma eq_natCast_of_transform_eqOn (hf : w ∈ ROC f) (hg : w ∈ ROC g)
    (heq : ∀ z : ℂ, ‖w‖ ≤ ‖z‖ → transform f z = transform g z) (n : ℕ) :
    f n = g n := by
...
lemma eq_of_isCausal_of_transform_eqOn
    (hcf : IsCausal f) (hcg : IsCausal g) (hf : w ∈ ROC f) (hg : w ∈ ROC g)
    (heq : ∀ z : ℂ, ‖w‖ ≤ ‖z‖ → transform f z = transform g z) :
    f = g := by
  funext n
