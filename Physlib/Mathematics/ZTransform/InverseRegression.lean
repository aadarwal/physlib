...
The theorems exercised here are Theorem 15 (inverse z-transform), Theorem 16 (uniqueness), and
...
pp. 875-907, as reformulated in `Physlib.Mathematics.ZTransform.Inverse`.
...
theorem transform_shiftedImpulse_eq (z : ℂ) :
...
theorem shiftedImpulse_ne_unitImpulse : shiftedImpulse ≠ unitImpulse := by
  intro hcon
...
theorem shiftedImpulse_not_isCausal : ¬ IsCausal shiftedImpulse := by
  intro hc
...
/-- The point `2 * a` lies in the region of convergence of a geometric sequence with nonzero
ratio `a`. -/
lemma two_mul_mem_ROC_geometric {a : ℂ} (ha : a ≠ 0) : 2 * a ∈ ROC (geometricSeq a) := by
...
theorem tendsto_transform_geometricSeq {a : ℂ} (ha : a ≠ 0) :
...
theorem eq_geometricSeq_of_transform_eq {a : ℂ} (ha : a ≠ 0) {h : ℤ → ℂ} {w : ℂ}
