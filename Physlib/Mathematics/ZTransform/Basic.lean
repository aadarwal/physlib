...
This file develops the unilateral Z-transform of a two-sided complex sequence `f : ℤ → ℂ` as the
analytic series `∑' n : ℕ, f n * z⁻¹ ^ n`. Only the nonnegative-index samples enter the series, so
the transform is unilateral even though the sequence itself is two-sided. Keeping the negative
indices in the domain is what allows the delay and advance laws below to be stated without
silently discarding startup data: the advance law produces an explicit finite startup sum, and the
delay law is stated under an explicit causality hypothesis rather than assumed.
...
- `Physlib.ZTransform.transform_delay`: the right-shift law `z⁻¹ ^ m * transform f z` for a causal
  sequence.
...
lemma transform_eq_tsum (f : ℤ → ℂ) (z : ℂ) : transform f z = ∑' n : ℕ, f n * z⁻¹ ^ n := rfl
