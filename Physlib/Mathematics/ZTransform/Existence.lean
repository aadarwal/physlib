...
The consequence is that the transfer-function statements of the previous file no longer depend
on a supplied solution. A causal solution now exists and is unique, so the transfer relation is a
statement about the constructed solution of the difference equation. Its summability and
nonvanishing-denominator hypotheses remain explicit.
...
/-- The nonnegative-index samples of a candidate causal solution with feedback lags `s`, feedback
coefficients `α`, and driving sequence `d`. The guard on each lag is what makes the recursion
decrease. Correctness as the stated recurrence requires the later hypothesis `0 ∉ s`. -/
def solveNat (s : Finset ℕ) (α : ℕ → ℂ) (d : ℤ → ℂ) : ℕ → ℂ
  | n =>
      (∑ k ∈ s, α k *
        (if _hk : 0 < k ∧ k ≤ n then solveNat s α d (n - k) else 0)) + d n
  decreasing_by omega

/-- The recursively constructed candidate extended by zero to negative indices. It is proved to
solve the strictly causal difference equation when `0 ∉ s` and the input is causal. -/
...
lemma isRecurrenceSolution_recurrenceSolution (h0 : 0 ∉ s) (hx : IsCausal x) :
  ...
  · obtain ⟨m, rfl⟩ := Int.eq_ofNat_of_zero_le hn
    ...
    rw [hlhs, solveNat, delayCombination]
    congr 1
    ...
    · have hnn : (0 : ℤ) ≤ (m : ℤ) - (k : ℤ) := by omega
      ...
      rw [dif_pos ⟨hkpos, hkm⟩, recurrenceSolution, zeroExtend, if_pos hnn, htn]
...
solution. -/
lemma existsUnique_causal_isRecurrenceSolution (h0 : 0 ∉ s) (hx : IsCausal x) :
...
lemma eq_recurrenceSolution {y : ℤ → ℂ} (h0 : 0 ∉ s) (hx : IsCausal x) (hy : IsCausal y)
...
supplied. -/
lemma transform_recurrenceSolution_cleared {z : ℂ} (h0 : 0 ∉ s) (hx : IsCausal x)
...
lemma transform_recurrenceSolution {z : ℂ} (h0 : 0 ∉ s) (hx : IsCausal x)
    (hxs : Summable (seriesTerm x z))
    (hys : Summable (seriesTerm (recurrenceSolution s t α β x) z))
