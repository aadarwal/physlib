...
operator, with an explicit bound. That direction is proved. **No converse is claimed here.** For
the total `tsum`-based convolution used below, a converse would first require convergence-aware
operator semantics as well as the usual sign-selecting-input or uniform-gain argument. Nothing
here should be read as an equivalence.
...
- `Physlib.ZTransform.isBIBOStable_of_isAbsSummable`: absolute summability implies
  bounded-input bounded-output stability. No converse is claimed.
...
lemma isAbsSummable_iff_one_mem_ROC (h : ℤ → ℂ) :
...
lemma isAbsSummable_iff_closedExterior_subset_ROC (h : ℤ → ℂ) :
...
convergence. -/
lemma isAbsSummable_iff_sphere_subset_ROC (h : ℤ → ℂ) :
...
lemma norm_convolution_le (hh : IsAbsSummable h) {C : ℝ}
    (hC : ∀ n : ℤ, ‖x n‖ ≤ C) (n : ℤ) :
...
/-- Absolute summability implies bounded-input bounded-output stability. No converse is claimed
for this total `tsum`-based convolution. -/
lemma isBIBOStable_of_isAbsSummable (hh : IsAbsSummable h) : IsBIBOStable h := by
  rintro x ⟨C, hC⟩
...
lemma isBIBOStable_of_sphere_subset_ROC (h : ℤ → ℂ)
...
circle. -/
def IsSchurStable (s : Finset ℕ) (α : ℕ → ℂ) : Prop :=
  ∀ z ∈ candidatePoles s α, ‖z‖ < 1

/-- Under Schur stability the denominator symbol does not vanish anywhere on the closed exterior
of the unit circle. -/
lemma denominator_ne_zero_of_isSchurStable
    (hS : IsSchurStable s α) {z : ℂ} (hz : 1 ≤ ‖z‖) :
...
lemma closedExterior_subset_iirROC_of_isSchurStable (hS : IsSchurStable s α)
...
lemma transform_eq_transferFunction_mul_of_isSchurStable (hcx : IsCausal x) (hcy : IsCausal y)
...
lemma isSchurStable_of_sum_norm_lt_one
    (hsum : ∑ k ∈ s, ‖α k‖ < 1) : IsSchurStable s α := by
  rintro z ⟨hne, hzero⟩
  by_contra hge
  rw [not_lt] at hge
  have hinv : ‖z⁻¹‖ ≤ 1 := by
