...
lemma apply_mem_support_of_ne {σ : Equiv.Perm ι} {t : ι} (h : σ t ≠ t) :
    σ t ∈ σ.support := by
...
lemma getLast_orbitPath {σ : Equiv.Perm ι} {t : ι} (h : σ t ≠ t) :
  ...
  have hmem : (orbitPath σ t).getLast (orbitPath_ne_nil σ t) ∈ orbitPath σ t :=
    List.getLast_mem _
  have hform :
      (orbitPath σ t).formPerm ((orbitPath σ t).getLast (orbitPath_ne_nil σ t)) = σ t := by
...
lemma formPerm_orbitPath_apply {σ : Equiv.Perm ι} {t x : ι}
    (hx : x ∈ (orbitPath σ t).toFinset) :
