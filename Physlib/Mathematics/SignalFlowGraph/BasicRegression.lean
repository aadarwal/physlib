...
criterion, and the nonvanishing hypothesis carried by every later theorem is load-bearing.
...
theorem systemMatrix_twoNodeLoop (a b : ℂ) :
...
theorem det_systemMatrix_twoNodeLoop (a b : ℂ) :
...
inverse. -/
theorem gain_twoNodeLoop (a b : ℂ) : gain (twoNodeLoop a b) 0 1 = a / (1 - a * b) := by
...
theorem det_systemMatrix_twoNodeLoop_one : (systemMatrix (twoNodeLoop 1 1)).det = 0 := by
...
theorem not_existsUnique_twoNodeLoop_one :
...
theorem isNodeSolution_twoNodeLoop_one_ones :
