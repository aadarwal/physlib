...
The file also supplies self-adjointness, inverse, trace, commutator, anticommutator, and
three-vector product identities. It contains no Relativity-specific structure; results using
Lorentz tensors belong under Relativity.

## Main definitions

- `pauliMatrix`: the identity and three standard Pauli matrices.
- `vectorMatrix`: the matrix `a · σ` associated to a real three-vector `a`.
-/
