...
The same fixture also checks the phase-vector law of the plane of incidence. Its incident,
reflected, and transmitted directions are `(3/5, 0, 4/5)`, `(3/5, 0, -4/5)`, and
`(4/5, 0, 3/5)`.
...
- `jonesBoundaryRegression_exact_phaseVectors_mem_incidencePlane`: the active reflected and
  transmitted phase vectors lie in the incident-vector/interface-normal span.
...
- `fresnelInterferenceRegression_outOfPlaneZero_planeOfIncidenceGuard`: an out-of-plane dummy
  reflected phase vector violates the span law and exercises the zero-field guard.
...
/-- On the exact active `(3/5, 0, 4/5)` incidence fixture, both the reflected and transmitted
phase vectors lie in the real span of the incident phase vector and interface normal. -/
lemma jonesBoundaryRegression_exact_phaseVectors_mem_incidencePlane :
    let configuration :=
      jonesBoundaryRegressionConfiguration (5 / 11) (16 / 11) (-1 / 5) (8 / 5)
  ...
  have hPhase := jonesBoundaryRegression_exact_isFixedFrequencyElectricBoundary.1
  exact
    hPhase.reflected_electricAmplitude_eq_zero_or_phaseVectors_mem_incidencePlane.resolve_left
...
/-- A zero reflected carrier whose dummy phase vector is the transverse `(0, 1, 0)` axis, outside
the incidence span generated by the exact fixture's incident phase vector and interface normal. -/
def fresnelInterferenceRegressionOutOfPlaneZeroReflectedWave :
    ComplexMonochromaticPlaneWave where
  angularFrequency := 2
  angularFrequency_pos := by norm_num
  waveVector := ComplexWaveVector.ofReal jonesBoundaryRegressionAxisZero
  electricAmplitude := 0

/-- The exact incident and transmitted carrier paired with an out-of-plane, zero-electric-field
dummy reflected label. -/
def fresnelInterferenceRegressionOutOfPlaneZeroConfiguration :
  ...
  reflected := fresnelInterferenceRegressionOutOfPlaneZeroReflectedWave
  transmitted := jonesBoundaryRegressionIncidentWave jonesBoundaryRegressionIncidentJones

/-- The out-of-plane zero-reflection fixture is phase matched through the guarded zero-field
branch, while its active incident and transmitted labels agree definitionally. -/
lemma fresnelInterferenceRegressionOutOfPlaneZero_isElectricPhaseMatched :
    fresnelInterferenceRegressionOutOfPlaneZeroConfiguration.IsElectricPhaseMatched :=
  ⟨⟨rfl, rfl⟩, Or.inl rfl⟩

/-- The deliberately dummy reflected phase vector does not lie in the exact fixture's physical
incident-vector/interface-normal span. -/
lemma fresnelInterferenceRegressionOutOfPlaneZero_phaseVector_not_mem_incidencePlane :
    let configuration := fresnelInterferenceRegressionOutOfPlaneZeroConfiguration
    configuration.reflected.waveVector.phaseVector ∉
      Submodule.span ℝ
  ...
  have hIncident := jonesBoundaryRegression_incidentWave_isReferencedMaterialJonesWave
    jonesBoundaryRegressionIncidentJones
  dsimp only [fresnelInterferenceRegressionOutOfPlaneZeroConfiguration,
    fresnelInterferenceRegressionOutOfPlaneZeroReflectedWave,
    jonesBoundaryRegressionInterface]
  rw [ComplexWaveVector.phaseVector_ofReal, hIncident.waveVector_eq,
    ComplexWaveVector.phaseVector_ofReal, jonesBoundaryRegression_negativeMedium_waveSpeed]
  intro hMem
  rw [Submodule.mem_span_pair] at hMem
  rcases hMem with ⟨a, b, hCombination⟩
  have hCoordinate := congrArg (fun v : EuclideanSpace ℝ (Fin 3) ↦ v 1) hCombination
  norm_num [jonesBoundaryRegressionAxisZero, jonesBoundaryRegressionIncidentWave,
    ComplexMonochromaticPlaneWave.ofReal_angularFrequency,
    JonesVector.toMaterialPlaneWave_angularFrequency, jonesBoundaryRegressionIncidentFrame,
    PolarizationFrame.propagationVector, jonesBoundaryRegressionIncidentDirection,
    OrientedAffineHyperplane.normalVector, jonesBoundaryRegressionPlane,
    jonesBoundaryRegressionNormal] at hCoordinate

/-- The out-of-plane dummy reflected phase vector violates the unguarded span law, while the
source-named affine-coplanarity corollary remains applicable through its zero-field guard. -/
lemma fresnelInterferenceRegression_outOfPlaneZero_planeOfIncidenceGuard :
    let configuration := fresnelInterferenceRegressionOutOfPlaneZeroConfiguration
      ...
      (configuration.reflected.electricAmplitude = 0 ∨
        Coplanar ℝ
          {0,
            ...
            configuration.transmitted.waveVector.phaseVector,
            configuration.interface.plane.normalVector}) := by
  have hPhase := fresnelInterferenceRegressionOutOfPlaneZero_isElectricPhaseMatched
  exact ⟨rfl, fresnelInterferenceRegressionOutOfPlaneZero_phaseVector_not_mem_incidencePlane,
    hPhase.reflected_electricAmplitude_eq_zero_or_phaseVectors_coplanar⟩
